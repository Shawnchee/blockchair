...
df = pd.read_csv("charity_donor.csv", encoding="latin1")
...
async def predict(query: str = Query(..., description="The cause or interest")):
    """
    Get charity recommendations based on the user's interest or cause.
    
    - **query**: A string representing the user's interest (e.g., education, environment, health)
    
    Returns a list of recommended charities with relevance scores and match details.
    """
    if not query:
